...
from libra import (ObservationArchive, mask_simultaneous_transits_k296,
                   ...
                   mask_simultaneous_transits_k62, duration_to_otherparams)
...
run_name = 'k62_ngroups6_short' #'k62_ngroups6_photonnoise' #'k62_ngroups6_10transits'
...
for i in range(0, n_iterations):
    with ObservationArchive(run_name + '_' + planet, 'a') as obs:
        ...
        class MeanModel(Model):
            parameter_names = ['amp', 'depth', 't0', 'duration', 'b']

            def get_value(self, t):
                params = deepcopy(original_params)

                a_rs, inc = duration_to_otherparams(params.per, self.duration,
                                                    self.b, np.sqrt(self.depth),
                                                    params.ecc, params.w)
...
#        yerr = obs_err
        ...
        def log_prior(params):
            ...
            log_omegaS0, log_omega0, log_Q, amp, depth, t0, duration, b = params
            ...
            if not ((parameter_bounds['amp'][0] < amp < parameter_bounds['amp'][1])
                    ...
                    and (parameter_bounds['duration'][0] < duration < parameter_bounds['duration'][1])
                    and (parameter_bounds['b'][0] < b < parameter_bounds['b'][1])):
                return -np.inf
        ...
        try:
            ...
            samples_duration = sampler.flatchain[:, 5]
            samples_b = sampler.flatchain[:, 6]
            ...
            dset3 = group.create_dataset("amp", data=samples_amp,
                                         compression='gzip')
            dset4 = group.create_dataset("duration", data=samples_duration,
                                         compression='gzip')
            dset5 = group.create_dataset("b", data=samples_b,
                                         compression='gzip')
            obs.archive.flush()
